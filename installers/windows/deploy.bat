...
echo Copy the base installation control file
copy %toolDir%buildInstall.iss %shadowBuildDir% || exit 1
...
echo Running gawk to extract build info.
echo ; This script auto-generated by DEPLOY.BAT >%shadowBuildDir%\buildinfo.iss
gawk -f %toolDir%getBuildInfo.awk %sourcedir%\version.h >>%shadowBuildDir%\buildInfo.iss || exit 1
gawk -f %toolDir%getBuildInfo.awk %sourcedir%\build_number.h >>%shadowBuildDir%\buildInfo.iss || exit 1
gawk -f %toolDir%getBuildInfo.awk %sourcedir%\git_info.h >>%shadowBuildDir%\buildInfo.iss || exit 1
echo %shadowBuildDir% | gawk -f %toolDir%getBuildInfo.awk >>%shadowBuildDir%\buildInfo.iss || exit 1
echo #define MySuffix "%1" >>%shadowBuildDir%\buildinfo.iss
echo #define MySourceDir "%sourcedir%" >>%shadowBuildDir%\buildinfo.iss

echo Create Release directory and subdirectories
if exist %shadowBuildDir%\Release\*.* rmdir /s /q %shadowBuildDir%\Release
mkdir %shadowBuildDir%\Release
cd %shadowBuildDir%\Release || exit 1
copy %shadowBuildDir%\oscar\oscar.exe . || exit 1

::: Now in Release subdirectory
echo Copy Help...
if exist Help\*.* rmdir /s /q Help
mkdir Help
copy ..\oscar\help Help || exit 1

echo Copy Html...
if exist Html\*.* rmdir /s /q Html
mkdir Html
copy ..\oscar\html html || exit 1

if exist Translations\*.* rmdir /s /q Translations
mkdir Translations
copy ..\oscar\translations Translations || exit 1

::: Run deployment tool
windeployqt.exe --release --force --compiler-runtime OSCAR.exe || exit 1
...
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q BuildInstall.iss || exit 1
